...
import { X, Plus, Trash2, ChevronRight, ChevronLeft, Check, Package } from 'lucide-react';
import { Invoice, InvoiceItem, InvoiceTemplate, TemplateSettings, VatSettings, Product, Service } from '../types';
...
const PAYMENT_METHODS = ['Bank Transfer', 'Paystack', 'Stripe', 'Cash', 'Cheque', 'Other'];
const CURRENCIES = [
...
export default function InvoiceCreator({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-stretch md:items-center justify-center bg-stone-950/80 backdrop-blur-sm p-0 md:p-4">
      <div className="w-full md:max-w-3xl bg-white md:rounded-2xl shadow-2xl flex flex-col max-h-screen md:max-h-[92vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-stone-100 bg-stone-50/50 shrink-0">
          <div className="text-xs text-stone-400 font-medium">
            {step === 2 && items.length > 0 && `${items.length} item${items.length !== 1 ? 's' : ''}`}
            {step === 3 && <span className="font-mono font-bold text-stone-600">{fmt(total)}</span>}
          </div>
          <div className="flex gap-3">
            {step > 1 && (
              <button onClick={() => setStep(s => (s - 1) as Step)}
                className="flex items-center gap-1.5 px-4 py-2.5 bg-stone-100 hover:bg-stone-200 text-stone-700 rounded-lg text-sm font-bold transition-colors">
                <ChevronLeft className="w-4 h-4" /> Back
              </button>
            )}
            {step === 1 && (
              <button onClick={onClose}
                className="px-4 py-2.5 bg-stone-100 hover:bg-stone-200 text-stone-700 rounded-lg text-sm font-bold transition-colors">
                Cancel
              </button>
            )}
            {step < 3 ? (
              <button
                ...
                className="flex items-center gap-1.5 px-5 py-2.5 bg-primary text-white rounded-lg text-sm font-bold hover:bg-[#C13A0E] transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                Continue <ChevronRight className="w-4 h-4" />
              </button>
            ) : (
              <button
                onClick={handleSave}
                className="flex items-center gap-2 px-5 py-2.5 bg-primary text-white rounded-lg text-sm font-black hover:bg-[#C13A0E] transition-colors shadow-md shadow-primary/20">
